...
ex_callback(rb_event_flag_t event, const rb_internal_thread_event_data_t *event_data, void *user_data)
{
    ...
    switch (event) {
      case RUBY_INTERNAL_THREAD_EVENT_STARTED:
        if (last_event != 0) {
            rb_bug("TestThreadInstrumentation: `started` event can't be preceded by `%s`", event_name(last_event));
        }
        break;
      case RUBY_INTERNAL_THREAD_EVENT_READY:
        if (last_event != 0 && last_event != RUBY_INTERNAL_THREAD_EVENT_STARTED && last_event != RUBY_INTERNAL_THREAD_EVENT_SUSPENDED) {
            rb_bug("TestThreadInstrumentation: `ready` must be preceded by `started` or `suspended`, got: `%s`", event_name(last_event));
        }
        break;
      case RUBY_INTERNAL_THREAD_EVENT_RESUMED:
        if (last_event != 0 && last_event != RUBY_INTERNAL_THREAD_EVENT_READY) {
            rb_bug("TestThreadInstrumentation: `resumed` must be preceded by `ready`, got: `%s`", event_name(last_event));
        }
        break;
      case RUBY_INTERNAL_THREAD_EVENT_SUSPENDED:
        if (last_event != 0 && last_event != RUBY_INTERNAL_THREAD_EVENT_RESUMED) {
            rb_bug("TestThreadInstrumentation: `suspended` must be preceded by `resumed`, got: `%s`", event_name(last_event));
        }
        break;
      case RUBY_INTERNAL_THREAD_EVENT_EXITED:
        if (last_event != 0 && last_event != RUBY_INTERNAL_THREAD_EVENT_RESUMED) {
            rb_bug("TestThreadInstrumentation: `exited` must be preceded by `resumed`, got: `%s`", event_name(last_event));
        }
        break;
    }
