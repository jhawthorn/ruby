...
static uint32_t
yjit_force_iv_index(VALUE klass, ID name)
{
    ASSERT_vm_locking();
...
gen_set_ivar(jitstate_t *jit, ctx_t *ctx, VALUE klass, ID ivar_name)
{
    ...
    uint32_t ivar_index = yjit_force_iv_index(klass, ivar_name);
...
gen_get_ivar(jitstate_t *jit, ctx_t *ctx, const int max_chain_depth, VALUE comptime_receiver, ID ivar_name, insn_opnd_t reg0_opnd, uint8_t *side_exit)
{
    ...
    */
    ...
    uint32_t ivar_index = yjit_force_iv_index(CLASS_OF(comptime_receiver), ivar_name);
